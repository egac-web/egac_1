...
async function createInviteForEnquiry(enquiry_id) {
  const client = getSupabaseAdmin();
...
async function appendEnquiryEvent(enquiry_id, event) {
  const client = getSupabaseAdmin();
...
async function getLatestInviteForEnquiry(enquiry_id) {
  const client = getSupabaseAdmin();
...
async function markEnquiryPresliConfirmed(enquiry_id, note) {
  const client = getSupabaseAdmin();
...
async function markInviteSent(invite_id) {
  const client = getSupabaseAdmin();
...
async function markInviteAccepted(invite_id) {
  const client = getSupabaseAdmin();
...
async function countBookingsForDateSlot(session_date, slot) {
  const client = getSupabaseAdmin();
  const { count, error } = await client.from("bookings").select("id", { count: "exact", head: true }).eq("session_date", session_date).eq("slot", slot);
  if (error) throw error;
  return count || 0;
}
async function createBooking(enquiry_id, invite_id, session_date, slot, session_time) {
  const client = getSupabaseAdmin();
...
async function getBookingByInvite(invite_id) {
  const client = getSupabaseAdmin();
...
export { appendEnquiryEvent, cancelBooking, countBookingsForDateSlot, createBooking, createInviteForEnquiry, getSupabaseAdmin as default, getBookingByInvite, getInviteByToken, getLatestInviteForEnquiry, getSupabaseAdmin, insertEnquiry, markEnquiryPresliConfirmed, markInviteAccepted, markInviteSent };
