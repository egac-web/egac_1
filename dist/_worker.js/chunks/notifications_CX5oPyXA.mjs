...
async function sendInviteNotification({ enquiryId, inviteId, to, inviteUrl, env }) {
  ...
  if (!env?.RESEND_API_KEY || !env?.RESEND_FROM) {
    try {
      ...
      console.error("Failed to append invite_send_failed", e);
    }
  ...
  const stagingAllowed = (env?.STAGING_ALLOWED_EMAILS || process.env.STAGING_ALLOWED_EMAILS || "").split(",").map((s) => s.trim()).filter(Boolean);
  const stagingRedirect = (env?.STAGING_REDIRECT_EMAILS || process.env.STAGING_REDIRECT_EMAILS || "").split(",").map((s) => s.trim()).filter(Boolean);
  if (appEnv === "staging") {
    ...
    if (!isAllowed) {
      ...
      try {
        await appendEnquiryEvent(enquiryId, { type: "staging_email_redirected", invite_id: inviteId, original_to: to, redirected_to: effectiveTo, at: nowISO() }, env);
      } catch (e) {
        console.error("Failed to append staging_email_redirected", e);
      }
    } else {
  ...
  if (dry) {
    try {
      ...
      console.error("Failed to append invite_send_dry_run", e);
    }
  ...
  try {
    ...
    try {
      ...
      console.error("Failed to append enquiry event after invite_sent", e);
    }
    try {
      await markInviteSent(inviteId, env);
    } catch (e) {
      console.error("Failed to mark invite sent", e);
    }
...
async function sendBookingConfirmationNotification({ enquiryId, bookingId, to, date, slotLabel, env }) {
  const dry = !!env?.RESEND_DRY_RUN;
  if (!env?.RESEND_API_KEY || !env?.RESEND_FROM) {
    try {
      await appendEnquiryEvent(enquiryId, { type: "booking_confirm_email_failed", booking_id: bookingId, error: "Resend not configured", at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append booking_confirm_email_failed", e);
    }
    return { ok: false, error: "no_resend_config" };
  }
  const appEnv = env?.APP_ENV || process.env.APP_ENV || "production";
  ...
  if (appEnv === "staging") {
    ...
    if (!isAllowed) {
      ...
      try {
        await appendEnquiryEvent(enquiryId, { type: "staging_email_redirected", booking_id: bookingId, original_to: to, redirected_to: effectiveTo, at: nowISO() }, env);
      } catch (e) {
        console.error("Failed to append staging_email_redirected", e);
      }
    } else {
  ...
  if (dry) {
    try {
      await appendEnquiryEvent(enquiryId, { type: "booking_confirm_email_dry_run", booking_id: bookingId, to: effectiveTo, at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append booking_confirm_email_dry_run", e);
    }
    return { ok: true, dryRun: true };
  }
  try {
    ...
    try {
      await appendEnquiryEvent(enquiryId, { type: "booking_confirm_email_sent", booking_id: bookingId, resend_id: res.id, at: nowISO(), meta: res.raw, template_key: "booking_confirmation" }, env);
    } catch (e) {
      console.error("Failed to append booking_confirm_email_sent event", e);
    }
    return { ok: true, resendId: res.id };
  } catch (err) {
    console.error("sendBookingConfirmation failed", err);
    try {
      await appendEnquiryEvent(enquiryId, { type: "booking_confirm_email_failed", booking_id: bookingId, error: err && err.response ? err.response : String(err), at: nowISO(), template_key: "booking_confirmation" }, env);
    } catch (e) {
      console.error("Failed to append booking_confirm_email_failed event", e);
    }
    return { ok: false, error: err };
  }
}
async function sendReminderNotification({ enquiryId, to, date, slotLabel, env }) {
  const dry = !!env?.RESEND_DRY_RUN;
  if (!env?.RESEND_API_KEY || !env?.RESEND_FROM) {
    try {
      await appendEnquiryEvent(enquiryId, { type: "reminder_email_failed", date, slotLabel, error: "Resend not configured", at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append reminder_email_failed", e);
    }
    return { ok: false, error: "no_resend_config" };
  }
  ...
  if (appEnv === "staging") {
    ...
    if (!isAllowed) {
      ...
      try {
        await appendEnquiryEvent(enquiryId, { type: "staging_email_redirected", original_to: to, redirected_to: effectiveTo, at: nowISO() }, env);
      } catch (e) {
        console.error("Failed to append staging_email_redirected for reminder", e);
      }
    } else {
  ...
  if (dry) {
    try {
      await appendEnquiryEvent(enquiryId, { type: "reminder_email_dry_run", date, slotLabel, to: effectiveTo, at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append reminder_email_dry_run", e);
    }
    return { ok: true, dryRun: true };
  }
  try {
    const res = await sendInviteEmail({ apiKey: env.RESEND_API_KEY, from: env.RESEND_FROM, to: effectiveTo, subject: effectiveSubject, html, text });
    try {
      await appendEnquiryEvent(enquiryId, { type: "reminder_email_sent", date, slotLabel, resend_id: res.id, at: nowISO(), meta: res.raw }, env);
    } catch (e) {
      console.error("Failed to append reminder_email_sent", e);
    }
    return { ok: true, resendId: res.id };
  } catch (err) {
    console.error("sendReminderNotification failed", err);
    try {
      await appendEnquiryEvent(enquiryId, { type: "reminder_email_failed", date, slotLabel, error: err && err.response ? err.response : String(err), at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append reminder_email_failed", e);
    }
    return { ok: false, error: err };
...
async function sendAcademyInvitation(invitation, enquiry, env) {
  const dry = !!env?.RESEND_DRY_RUN;
  if (!env?.RESEND_API_KEY || !env?.RESEND_FROM) {
    try {
      await appendEnquiryEvent(enquiry.id || invitation.enquiry_id, { type: "academy_invite_failed", invite_id: invitation.id, error: "Resend not configured", at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append academy_invite_failed", e);
    }
  ...
  if (appEnv === "staging") {
    ...
    if (!isAllowed) {
      ...
      try {
        await appendEnquiryEvent(enquiry.id || invitation.enquiry_id, { type: "staging_email_redirected", invite_id: invitation.id, original_to: enquiry.email, redirected_to: effectiveTo, at: nowISO() }, env);
      } catch (e) {
        console.error("Failed to append staging_email_redirected for academy", e);
      }
    } else {
      effectiveSubject = `[STAGING] ${subject}`;
    }
  ...
  if (dry) {
    try {
      await appendEnquiryEvent(enquiry.id || invitation.enquiry_id, { type: "academy_invite_dry_run", invite_id: invitation.id, to: effectiveTo, at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append academy_invite_dry_run", e);
    }
    return { success: true };
  }
  try {
    const res = await sendInviteEmail({ apiKey: env.RESEND_API_KEY, from: env.RESEND_FROM, to: effectiveTo, subject: effectiveSubject, html, text });
    try {
      await appendEnquiryEvent(enquiry.id || invitation.enquiry_id, { type: "academy_invite_sent", invite_id: invitation.id, resend_id: res.id, at: nowISO(), meta: res.raw }, env);
    } catch (e) {
      console.error("Failed to append academy_invite_sent", e);
    }
    return { success: true, resendId: res.id };
  } catch (err) {
    console.error("sendAcademyInvitation failed", err);
    try {
      await appendEnquiryEvent(enquiry.id || invitation.enquiry_id, { type: "academy_invite_failed", invite_id: invitation.id, error: err && err.response ? err.response : String(err), at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append academy_invite_failed", e);
    }
...
async function sendAcademyWaitlistNotification({ enquiry, invitation, env }) {
  const dry = !!env?.RESEND_DRY_RUN;
  if (!env?.RESEND_API_KEY || !env?.RESEND_FROM) {
    try {
      await appendEnquiryEvent(enquiry.id || invitation.enquiry_id, { type: "academy_waitlist_notification_failed", invite_id: invitation.id, error: "Resend not configured", at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append academy_waitlist_notification_failed", e);
    }
    return { ok: false, error: "no_resend_config" };
  }
  const siteBase = env?.SITE_BASE_URL || process.env.SITE_BASE_URL || "";
  ...
  if (appEnv === "staging") {
    ...
    if (!isAllowed) {
      ...
      try {
        ...
        console.error("Failed to append staging_email_redirected for academy waitlist", e);
      }
    } else {
      effectiveSubject = `[STAGING] ${subject}`;
    }
  ...
  if (dry) {
    try {
      await appendEnquiryEvent(enquiry.id || invitation.enquiry_id, { type: "academy_waitlist_notification_dry_run", invite_id: invitation.id, to: effectiveTo, at: nowISO() }, env);
    } catch (e) {
      console.error("Failed to append academy_waitlist_notification_dry_run", e);
    }
    return { ok: true, dryRun: true };
  }
  try {
    const res = await sendInviteEmail({ apiKey: env.RESEND_API_KEY, from: env.RESEND_FROM, to: effectiveTo, subject: effectiveSubject, html, text });
    try {
      await appendEnquiryEvent(enquiry.id || invitation.enquiry_id, { type: "academy_waitlist_notification_sent", invite_id: invitation.id, resend_id: res.id, at: nowISO(), meta: res.raw, template_key: "academy_waitlist" }, env);
    } catch (e) {
      console.error("Failed to append academy_waitlist_notification_sent", e);
    }
    return { ok: true, resendId: res.id };
  } catch (err) {
    console.error("sendAcademyWaitlistNotification failed", err);
    try {
      await appendEnquiryEvent(enquiry.id || invitation.enquiry_id, { type: "academy_waitlist_notification_failed", invite_id: invitation.id, error: err && err.response ? err.response : String(err), at: nowISO(), template_key: "academy_waitlist" }, env);
    } catch (e) {
      console.error("Failed to append academy_waitlist_notification_failed", e);
    }
    return { ok: false, error: err };
...
export { sendAcademyInvitation, sendAcademyWaitlistNotification, sendBookingConfirmationNotification, sendInviteNotification, sendReminderNotification };
