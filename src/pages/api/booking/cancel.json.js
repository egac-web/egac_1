...
export async function POST({ request, locals }) {
  try {
    ...
    // append event
    try { await appendEnquiryEvent(invite.enquiry_id, { type: 'booking_cancelled', booking_id, at: new Date().toISOString() }, env); } catch (err) { console.error('append event failed', err); }
    ...
    if (env.RESEND_API_KEY && env.RESEND_FROM) {
      try {
        ...
        try { await appendEnquiryEvent(invite.enquiry_id, { type: 'booking_cancel_email_sent', booking_id, resend_id: res.id, at: new Date().toISOString(), meta: res.raw }, env); } catch (err) { console.error('append event failed', err); }
